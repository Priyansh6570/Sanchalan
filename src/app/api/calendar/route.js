...
export async function GET() {
  try {
    ...
    series.forEach(s => {
      // Episode schedule
      if (s.episodeUploadDay && s.episodeUploadTime) {
        const dayIndex = daysOfWeek.indexOf(s.episodeUploadDay)
        if (dayIndex !== -1) {
          ...
          if (nextDate <= nextWeek) {
            const [hours, minutes] = s.episodeUploadTime.split(':')
            nextDate.setHours(parseInt(hours), parseInt(minutes), 0)
            
            events.push({
              id: `episode-${s._id}-${nextDate.getTime()}`,
      ...
      // Trailer schedule
      if (s.trailerUploadDay && s.trailerUploadTime) {
        const dayIndex = daysOfWeek.indexOf(s.trailerUploadDay)
        if (dayIndex !== -1) {
          const nextDate = new Date(now)
          const daysUntil = (dayIndex - nextDate.getDay() + 7) % 7
          nextDate.setDate(nextDate.getDate() + (daysUntil === 0 ? 7 : daysUntil))
          ...
          if (nextDate <= nextWeek) {
            const [hours, minutes] = s.trailerUploadTime.split(':')
            nextDate.setHours(parseInt(hours), parseInt(minutes), 0)
            
            events.push({
              id: `trailer-${s._id}-${nextDate.getTime()}`,
